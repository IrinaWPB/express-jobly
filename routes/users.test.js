...
describe("GET /users/:username", function () {
  ...
  test("works for profile owners", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  ...
  test("works for owner of profile: set new password", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          password: "new-password",
...
describe("DELETE /users/:username", function () {
  ...
  test("works for owner of profile", async function () {
    const resp = await request(app)
        .delete(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ deleted: "u1" });
  ...
  test("fails for non-admin non-owner of profile", async function () {
    const resp = await request(app)
        .delete(`/users/u2`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("POST /users/:username/jobs/:id", () => {
  ...
  test("works for profile owners", async function() {
    let jobId = await request(app).get(`/jobs/j2`)
    jobId = jobId.body.job.id
    const resp = await request(app)
        .post(`/users/u1/jobs/${jobId}`)
        .set("authorization", `Bearer ${u1Token}`);
